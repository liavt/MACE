...
namespace mc{
	namespace gfx{
		...
		public:
			...
			void draw(GLenum type, Index offset);
		...
		class ShaderProgram {
			...
			std::map<char*, Index> uniforms = std::map<char*, Index>();
		...
		public:
			...
			Index getUniformLocation(char * name);
			...
			#define _CREATE_UNIFORM_MATRIX_FUNCTION(T,suffix,W,H) \
				...
				void setUniform(char * name, const mc::Matrix<T,W,H>& m){setUniform(name,false,m);}
			...
			#define _CREATE_UNIFORM_SQUARE_MATRIX_FUNCTION(T,suffix,N) \
				...
				void setUniform(char * name, const mc::Matrix<T,N,N>& m){setUniform(name,false,m);}
